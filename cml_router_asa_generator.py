...
def read_input_file(file_path):
    """
    input.txt dosyasını okuyarak bağlantıları ve router/switch'leri çıkarır.
    """
    connections = []
...
def calculate_distance(x1, y1, x2, y2):
    """
    İki nokta arasındaki mesafeyi hesaplar (Öklid mesafesi).
    """
    return math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)

def create_yaml_structure(connections, routers):
    """
    Router ve switch'lerin bağlantılarını içeren YAML yapısını oluşturur.
    """
    center_x, center_y = 0, 0  # Merkez router (R1) için sabit koordinatlar
    ...
    for i, router in enumerate(sorted_routers):
        ...
        if router.startswith('r'):
            ...
            label = f'SW{switch_counter}'  # Switch etiketi (SW1, SW2, ...)
            switch_counter += 1  # Switch sayacını artır
        elif router.lower().startswith('asa'):
            node_definition = 'asav'  # asa ile başlayan router'lar için
            label = router.upper()  # Router etiketi (ASA1, ASA2, ...)
            router_positions[router] = (x, y)  # Router'ın koordinatlarını kaydet
        ...
        elif node_definition == 'asav':
            # ASA cihazları için Management0/0 arayüzünü ekle, ancak bağlantı yaparken kullanma
            interface_labels = [
                {'id': 'i0', 'label': 'Loopback0', 'mac_address': None, 'type': 'loopback'},
                {'id': 'i1', 'label': 'Management0/0', 'mac_address': None, 'slot': 0, 'type': 'physical'},
                {'id': 'i2', 'label': 'GigabitEthernet0/0', 'mac_address': None, 'slot': 1, 'type': 'physical'},
                {'id': 'i3', 'label': 'GigabitEthernet0/1', 'mac_address': None, 'slot': 2, 'type': 'physical'},
                {'id': 'i4', 'label': 'GigabitEthernet0/2', 'mac_address': None, 'slot': 3, 'type': 'physical'}
            ]
    ...
    for conn in connections:
        ...
        # ASA cihazları için Management0/0 arayüzünü kullanma
        if conn['source_router'].lower().startswith('asa') and source_interface == 'Management0/0':
            print(f"Warning: Management0/0 interface cannot be used for connections. Skipping connection: {conn}")
            continue
        if conn['target_router'].lower().startswith('asa') and target_interface == 'Management0/0':
            print(f"Warning: Management0/0 interface cannot be used for connections. Skipping connection: {conn}")
            continue
...
def write_yaml_file(yaml_structure, file_path):
    """
    YAML yapısını bir dosyaya yazar.
    """
